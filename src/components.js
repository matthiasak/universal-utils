...
export const mobile_metas = (title='', img='icon', manifest='manifest') => [
    m('meta', {charset:'utf8'}),
...
export const fb_opengraph = (app_id, url, title, img, site_name, author) => [
    `fb:app_id,${app_id}`,
    `og:url,${url}`,
    `og:type,website`,
    `og:title,${title}`,
...
export const fb_instantarticle = (article_url, style) => [
...
export const twitter_card = (summary,site_account,individual_account,url,title,description,image) => [
...
export const google_plus = (page, title, desc, img) => [
...
export const iOS = (app_id,affiliate_id, app_arg, telephone='yes', title) =>
[
    // Smart App Banner
...
// Pinned Site - Safari
export const safari = (name='icon',color='red') => m('link', {rel:"mask-icon", href:`${name}.svg`, color})

// Disable translation prompt
export const chrome = (app_id) => [
    m('link', {rel:"chrome-webstore-item", href:`https://chrome.google.com/webstore/detail/${app_id}`}),
    m('meta', {name:'google', value:'notranslate'})
]

export const applinks = (app_store_id, name, android_pkg, docs_url) => [
    // iOS
...
export const viewportHeight = _ => Math.max(document.documentElement.clientHeight, window.innerHeight || 0)

export const trackVisibility = component => {
    let el,
...
export const markdown = (content, markdownToHtml=(c => global.markdeep.format(c))) => {
 ...
 */
export const chars = '#*^-+=!f0123456789_'
export const scramble = (str, from=0) =>
    str.slice(0,from) + str.slice(from).split('').map(x => x === ' ' ? x : chars[range(0,chars.length)]).join('')
export const range = (min,max) => Math.floor(Math.random()*(max-min)+min)
export const wait = ms => new Promise((res) => setTimeout(res, ms))

export const scrambler = (str, interval=33, i=0, delay=0) => el => {
 ...
 */
export const imageLoader = (url, comp) => {
    let x = comp,
        image,
        loaded = false
 ...
 */
export const loadSVG = url => fetch(url).then(r => r.text())
export const injectSVG = url => container(data =>
 ...
 */
export const page = (main, title, css='/style.css', googleAnalyticsId) => [
    head(
        theme(),
        mobile_metas(title),
 ...
 */
export const app = (routes, def, title, css, analyticsId) => {
